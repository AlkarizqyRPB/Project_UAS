 ...
 * @author ALKARIZQY
 */
public class FasilitasBDG {
    private final Connection koneksi;
    public FasilitasBDG() {
 koneksi = KoneksiDatabase.getKoneksi();
 }
 public void insert(Bandung barang){
 PreparedStatement prepare = null;
 try {
    String sql ="INSERT INTO bandung(No_Kursi,Nama,Jumlah,Tujuan,Harga)VALUES(?,?,?,?,?)";
 ...
 public void update(Bandung barang){
    PreparedStatement prepare = null;
 try {
 String sql ="UPDATE bandung SET Nama=?,jumlah=?,tujuan=?,harga=? WHERE No_Kursi=?";
    prepare = (PreparedStatement) koneksi.prepareStatement(sql);
 ...
 PreparedStatement prepare = null;
 try {
 String sql ="DELETE FROM bandung WHERE No_Kursi=?";
    prepare = (PreparedStatement) koneksi.prepareStatement(sql);
...
public List<Bandung> selectAll(){
 PreparedStatement prepare = null;
 ResultSet result = null;
 List<Bandung> list = new ArrayList<>();
 try {
 String sql ="SELECT * FROM bandung";
    prepare = (PreparedStatement) koneksi.prepareStatement(sql);
    result = prepare.executeQuery();
 while (result.next()){
    Bandung barang = new Bandung();
